...
class AdminController {
  ...
  static async loadDashboard(req, res) {
    ...
    const result = await userService.getUsers(search, page);
    const message = req.query.message;
    const error = req.query.error;
    res.render("admin/dashboard", { ...result, search, message, error });
  }

  static async addUser(req, res) {
    const { email, password, name, confirmPassword } = req.body;
    const result = await userService.addUser(email, password, name, confirmPassword);
    if (result.error) {
      return res.redirect(
  ...
  static async editUser(req, res) {
    ...
    const result = await userService.updateUser(id, email, password, confirmPassword, name);
    if (result.error) {
      return res.redirect(
        `/admin/dashboard?error=${encodeURIComponent(result.error)}`,
      );
