...
- [ ] **Update**
  ...
  - [ ] **Upsert** — `updateOne({ filter }, { $set: { data } }, { upsert: true })` — update if exists, insert if not
...
- [ ] **Index Types**
  ...
  - [ ] **Clustered vs Non-Clustered Indexes** — clustered = data stored in index order (faster reads, one per collection); non-clustered = separate structure pointing to data
...
- [ ] **Complete all MongoDB basics assignments**
  - [ ] Finish all practice problems from the assignment document
